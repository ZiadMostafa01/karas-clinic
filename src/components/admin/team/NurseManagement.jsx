...
export default function NurseManagement() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (formData.id) {
        await fetch(`${API_BASE_URL}/api/nurses/${formData.id}`, {
          method: "PUT",
          body: fd,
        });
      } else {
        await fetch(`${API_BASE_URL}/api/nurses`, {
          method: "POST",
          body: fd,
        });
      }
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API_BASE_URL}/api/nurses/${nurseToDelete.id}`, {
        method: "DELETE",
      });
          ...
          {nurses.map((nurse) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full overflow-hidden bg-teal-50 flex items-center justify-center text-teal-600 border border-teal-100">
                  {nurse.imageUrl ? (
                    <img
            ...
            <form className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="sm:col-span-2 flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="border border-gray-200 rounded-xl p-3 outline-none focus:border-blue-500 transition-all text-gray-800"
                  placeholder="Critical Care, Patient Support, etc."
                />
              </div>
