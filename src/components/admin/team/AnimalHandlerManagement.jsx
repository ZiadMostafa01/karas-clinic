...
export default function AnimalHandlerManagement() {
  ...
  const fetchHandlers = async () => { try { const res = await fetch(`${API_BASE_URL}/api/animalhandlers`); setHandlers(await res.json()); } catch (err) { console.error(err); } };
  useEffect(() => { fetchHandlers(); }, []);
  ...
  const handleEdit = (h) => { setFormData({ ...h, role: h.title || "", image: h.imageUrl ? `${API_BASE_URL}${h.imageUrl}` : null }); setImageFile(null); setIsModalOpen(true); };
  const handleDeleteClick = (h) => { setHandlerToDelete(h); setIsDeleteModalOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      const url = formData.id ? `${API_BASE_URL}/api/animalhandlers/${formData.id}` : `${API_BASE_URL}/api/animalhandlers`;
      await fetch(url, { method: formData.id ? "PUT" : "POST", body: fd });
      setIsModalOpen(false); fetchHandlers();
    } catch (err) { console.error(err); }
  };

  const handleDelete = async () => {
    try { await fetch(`${API_BASE_URL}/api/animalhandlers/${handlerToDelete.id}`, { method: "DELETE" }); setIsDeleteModalOpen(false); fetchHandlers(); } catch (err) { console.error(err); }
  };
    ...
    <div className=" font-sans">
      <div className=" rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-4 p-5 border-b border-gray-100 lg:p-6 justify-between items-center">
          <div>
            <h4 className="text-xl font-bold text-gray-800">Animal Handlers Directory</h4>
            <p className="text-sm text-gray-500">Total Handlers: {handlers.length}</p>
          </div>
        ...
        <div className="divide-y divide-gray-100">
          {handlers.map((handler) => (
            <div key={handler.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 gap-4">
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full overflow-hidden bg-orange-50 flex items-center justify-center text-orange-600 border border-orange-100">
                  {handler.imageUrl ? (<img src={`${API_BASE_URL}${handler.imageUrl}`} alt="" className="w-full h-full object-cover" />) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-md">{handler.title}</span>
                    <span className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-md">{handler.location}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button onClick={() => handleEdit(handler)} className="cursor-pointer text-sm font-bold text-gray-600 hover:bg-gray-100 py-2 px-4 rounded-lg hover:text-blue-600 transition-colors">Edit</button>
                <div className="h-4 w-px bg-gray-200"></div>
                <button onClick={() => handleDeleteClick(handler)} className="cursor-pointer text-sm font-bold text-red-500 hover:bg-red-100 py-2 px-4 rounded-lg hover:text-red-700 transition-colors">Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-[2px]">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="text-lg font-bold text-gray-800">{formData.id ? "Edit Handler Profile" : "Register New Handler"}</h3>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-gray-900/50">
          <div className="bg-white w-full max-sm:max-w-xs max-w-sm rounded-2xl p-8 text-center shadow-2xl border border-gray-100">
            ...
            <p className="text-sm text-gray-500 my-4 leading-relaxed">Are you sure you want to delete <span className="font-bold text-gray-700">"{handlerToDelete?.name}"</span>?</p>
